...
class NetwithLoss(torch.nn.Module):
    ...
    def forward(self, imgs, backgrounds, targets,adaptive_layer=None):
        ...
        diff_pred = adaptive_layer(pred) - adaptive_layer(pred_back)
        # if(adaptive_layer is not None):
        #     diff_pred=adaptive_layer(diff_pred)
        ...
        print(f"studnet shape {diff_pred.shape}---- teacher shape {diff_predT.shape}")
