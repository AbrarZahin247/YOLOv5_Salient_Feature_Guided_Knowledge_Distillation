...
def kl_mse_loss(feat_a,feat_b):
    cross=feature_cross_entropy(feat_a,feat_b)
    mse=feature_mse(feat_a,feat_b)
    return cross+mse
...
class ComputeLoss:
    ...
    def __call__(self, p, targets, student=None, teacher=None):  # predictions, targets, model
        ...
        dist_loss = torch.tensor(kl_mse_loss(student, teacher), device=self.device, dtype=torch.float32)
        # dist_loss = torch.tensor(feature_mse(student, teacher), device=self.device, dtype=torch.float32)
        dist_loss = dist_loss.mean()
        # print(dist_loss)
        # Compute the total loss
        total_loss = distillation_factor * dist_loss + lbox + lobj + lcls
