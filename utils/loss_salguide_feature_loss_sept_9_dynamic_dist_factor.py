...
class SalientDistillLoss(nn.Module):
    def __init__(self, student, teacher, device):
        ...
        self.dist_factor=1e-0
        self.epcoh_map50=[]

    def update_map50(self,map):
        self.epcoh_map50.append(map)
        self.update_dist_factor(self.epcoh_map50)

    def update_dist_factor(self,map_values,threshold=0.01):    
        # Check if the number of mAP values is a multiple of 5
        if len(map_values) >= 5 and len(map_values) % 5 == 0:
            # Calculate the average of change for the last 5 mAP values
            last_five_values = map_values[-5:]
            changes = [last_five_values[i] - last_five_values[i-1] for i in range(1, len(last_five_values))]
            average_change = sum(changes) / len(changes)
            # Check if the average of change is greater than 0.01
            if average_change > threshold:
                self.dist_factor=self.dist_factor/10
    ...
    def forward(self, imgs, targets, gt_masks):
        ...
        # Combine losses
        return loss + loss_smoothL1*self.dist_factor, loss_items