        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            ...
            with torch.cuda.amp.autocast(amp):
                ...
                if opt.teacher_weight:
                    ...
                    std_pred_masked, std_features_foreground= model(batch_masked_image, target=targets)  # forward
                    diff_foreground_std_feature=std_features-std_features_foreground
                    
                    
                    tech_pred,tech_features = teacher_model(imgs,target=targets)
                    tech_pred_masked, tech_features_foreground= teacher_model(batch_masked_image, target=targets)  # forward
                    diff_foreground_tech_feature=tech_features-tech_features_foreground
                    ...
                    loss, loss_items = compute_loss(std_pred,targets,stu_feature_adapt(diff_foreground_std_feature), diff_foreground_tech_feature.detach())  # loss scaled by batch_size
